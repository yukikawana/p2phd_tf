...
def ssd_multibox_layer(inputs,
    ...
    oloc_pred = slim.conv2d(net, num_loc_pred, [3, 3], scope='conv_loc')
    loc_pred = tf.reshape(oloc_pred,
                          tensor_shape(oloc_pred, 4)[:-1]+[num_anchors, 4])
    # Class prediction.
    num_cls_pred = num_anchors * num_classes
    ocls_pred = slim.conv2d(net, num_cls_pred, [3, 3], scope='conv_cls')
    cls_pred = tf.reshape(ocls_pred,
                          tensor_shape(ocls_pred, 4)[:-1]+[num_anchors, num_classes])
    return cls_pred, loc_pred, ocls_pred, oloc_pred
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse):
        ...
        with tf.variable_scope(end_point):
            ...
            end_points[end_point] = net
            end_point = 'block11'
            net = tf.image.resize_image_with_crop_or_pad(net, 3, 6)
            net = slim.conv2d(net, 256, [3, 3], scope='conv3x3', padding='VALID')
        end_points[end_point] = net
        ...
        for i, layer in enumerate(feat_layers):
            with tf.variable_scope(layer + '_box'):
                p, l, op, ol = ssd_multibox_layer(end_points[layer],
                                          num_classes,
                                          anchor_sizes[i],
                                          anchor_ratios[i],
                                          normalizations[i])
            predictions.append(prediction_fn(p))
            end_points["cls%d"%i]=op
            logits.append(p)
